...
def strategy_form(request, strategy_name):
    if request.method == 'POST':
        ...
        if endpoint and interval and strategy_name:
            task = create_scheduled_task(endpoint, interval, strategy_name, additional_params)
            return HttpResponse(f"Task for {endpoint} with strategy {strategy_name} scheduled successfully.")
        else:
            return HttpResponse("Missing required fields.", status=400)

    interval_choices = IntervalChoices.choices()
    return render(request, 'tasks/strategy_form.html', {'strategy_name': strategy_name, 'interval_choices': interval_choices})
